...
public class QueueDataProvider {
    ...
    public void removeAll() {
        synchronized (mLock) {
            ...
            remoteMediaClient.queueRemoveItems(remoteMediaClient.getMediaQueue().getItemIds(), null);
            mQueue.clear();
        }
    }
    ...
    private void syncWithRemoteQueue() {
		Log.d(TAG, "syncWithRemoteQueue()");
        RemoteMediaClient remoteMediaClient = getRemoteMediaClient();
        if (remoteMediaClient != null) {
            remoteMediaClient.addListener(mRemoteMediaClientListener);
            MediaStatus mediaStatus = remoteMediaClient.getMediaStatus();
            if (mediaStatus != null) {
                List<MediaQueueItem> items = mediaStatus.getQueueItems(); /*remoteQueueItems(remoteMediaClient);*/
    ...
    private class MyRemoteMediaClientListener implements RemoteMediaClient.Listener {
        ...
        private void updateMediaQueue() {
            ...
            if (remoteMediaClient != null) {
                ...
                if (mediaStatus != null) {
                    queueItems = mediaStatus.getQueueItems(); /*remoteQueueItems(remoteMediaClient);*/
                    mRepeatMode = mediaStatus.getQueueRepeatMode();
                    mCurrentIem = mediaStatus.getQueueItemById(mediaStatus.getCurrentItemId());
                }
            }
            if (queueItems == null) {
                ...
                Log.d(TAG, "media status is updated with a list of size: " + queueItems.size());
    ...
    /*private List<MediaQueueItem> remoteQueueItems(RemoteMediaClient remoteMediaClient)
    {
        List<MediaQueueItem> queueItems = null;
        if (remoteMediaClient != null) {
            ...
            if (mediaQueue != null) {
                ...
                for (int i = 0; i < mediaQueue.getItemCount(); i++) {
                    queueItems.add(mediaQueue.getItemAtIndex(i, true));
                }
            }
        }
        return queueItems;
